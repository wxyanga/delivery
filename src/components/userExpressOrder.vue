			...
			<el-tab-pane label="批量录入" name="second">
				<el-form label-width="150px">
					...
					<el-form-item label="快递类型">
						<el-radio-group v-model="separateSelected" @change="changeSeparate">
							<el-radio-button
								v-for="(it, ind) in separateExpByType"
								:key="ind"
								:label="it.map"
				...
				>
					<el-form-item label="选择快递类型" prop="ExpTypeLabel" style="width:80%">
						<el-select
							v-model="validatorForm.ExpTypeLabel"
							style="width:100%"
							@change="changeSeparateLabel"
						>
					...
					<el-form-item label="录入参照格式">
						...
						<div>
							<span
								style="background:green;color:#fff;padding:5px 10px;margin-right:10px"
							>收货人姓名 收货人电话 收货地址(或省,市,区,详细地址) 电商平台订单号（订单号不是必填）</span>
							<span style="color:red">中间使用空格,位置必须严格一致,每条信息间使用回车换行</span>
						</div>
					...
					<el-form-item label="单据录入" style="width:80%">
						...
						<el-button
							type="primary"
							style="float: right;"
							:disabled="!setDefaultExpType.id || ExpTable.length == 0"
							@click="upEXP"
						>
							共 {{ ExpTable.length }} 个信息，下单需使用 {{ ((setDefaultExpType.myMoney || 0) * (ExpTable.length || 0)).toFixed(2) }} 元，账户余额：{{
							userInfo.bal
							}}，确认金额并下单
						</el-button>
					</el-form-item>
				...
				<div style="width:60%;">
					<el-form :inline="true">
						...
						<el-form-item label>
							<a
								v-if="!isIE()"
								style="margin-left: .5rem;margin-top: .5rem;float:left"
								type="text"
								download="模板.xlsx"
								:href="templateUrl"
							>模板下载</a>
